...
- No tabs
- Uses `prettier-plugin-packagejson` to sort `package.json` fields
...
- All concrete API classes extend `BaseAPI<TResponse, TModel>` (`base.api.ts`), which holds `protected readonly client: NotionClient` and provides shared methods: `retrieveResource`, `createResource`, `updateResource`, `deleteResource`, `listResources`, `parseAndWrap`, `parsePaginatedList`
- Constructors call `super(client)` to pass the `NotionClient` to `BaseAPI`
...
| --------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NotionAPIError`            | HTTP error responses              | `status`, `code`, `body`, `retryAfterMs`; helpers: `isRateLimited()`, `isNotFound()`, `isUnauthorized()`, `isValidationError()`, `isServerError()`, `isRetryable()` |
...
| `eslint-plugin-zod`            | Zod-specific ESLint rules                     |
| `prettier`                     | Code formatting                               |
| `prettier-plugin-packagejson`  | Sorts `package.json` fields via Prettier      |
...
- **Husky hooks:**
  ...
  - `post-checkout`: Runs `bun install` when `package.json` or `bun.lock` change on branch switch
- **Commit message format:** Conventional Commits (e.g., `feat:`, `fix:`, `chore:`, `refactor:`, `docs:`, `test:`)
- **Lint-staged:** Auto-fixes and formats `.ts`, `.json`, `.md`, `.yml`, `.yaml` files on commit
...
- `prefer-arrow-callback`, `prefer-template` (warn), `object-shorthand` (warn)
...
- `eslint-plugin-zod` -- Zod-specific rules via `zod.configs.recommended`
